...
const InfoSection = forwardRef(function InfoSection(props, ref) {
    ...
    if (props.image) {
        additionalContent = <Col md={4} className='p-4 text-center .d-sm-none .d-md-block'>
            <img className='img-fluid rounded-3 border' style={{height: '400px'}} src={props.image} alt='image'/>
            </Col>
    } 
    ...
    if (props.button) {
        additionalButton = (
            <Button 
                ...
                style={{ marginTop: '15px', paddingLeft: '8px', paddingRight: '8px', paddingRight: '15px' }} 
            ...
            > <span className='recolor-image-h'> 
                <img
                    src={PlusImage} 
                    alt="plus" 
                    style={{ marginRight: '6px', marginLeft: '5px', verticalAlign: 'middle', position: 'relative', top: '-1px', opacity: '0.6' }}
    ...
    return (
        <div className='p-1'>
            <Container style={{minHeight: '400px'}} className= {`my-4 bg-light rounded-4 ${props.shadow ? 'shadow' : ''}`} > 
                <Row>
                    <Col className='p-5'>
